...
function targetArgument(value) {
    if (!value || value.startsWith("-") || /[\u0000-\u001f\u007f]/.test(value)) {
        throw new Error("target is missing or invalid");
    }
    return value;
}
function parseCommon(argv, allowMode = true) {
    const rest = [];
    const options = { mode: "all", timeoutMs: 120_000, readLines: 200 };
    for (let index = 0; index < argv.length; index += 1) {
        const value = argv[index];
        if (value === "--mode") {
            if (!allowMode)
                throw new Error("--mode is not supported by this command");
            const mode = argv[++index];
            if (mode !== "all" && mode !== "first")
                throw new Error("--mode must be all or first");
            options.mode = mode;
        }
...
function tool(name, definitions) {
...
function printResult(result, write) {
    const message = result.content.map((item) => item.text).join("\n");
    if (result.isError)
        throw new Error(message);
    write(`${message}\n`);
}
export function requireManagedCaller(environment = process.env) {
...
async function credentials(controllerId, dependencies) {
...
export async function main(argv = process.argv.slice(2), dependencies = defaultDependencies) {
    ...
    if (command === "status") {
        if (argv.length > 0)
            throw new Error("status accepts no arguments");
    ...
    if (command === "receipts") {
        if (argv.length > 0)
            throw new Error("receipts accepts no arguments");
    ...
    if (command === "ask-many") {
        ...
        if (rest.length > 0)
            throw new Error(`unknown ask-many arguments: ${rest.join(" ")}`);
    ...
    if (command === "collect") {
        ...
        for (let index = 0; index < argv.length; index += 1) {
            ...
            if (!receipt)
                throw new Error("--receipt requires a token");
            receipts.push(receipt);
        }
        const { rest, options } = parseCommon(common);
        if (rest.length > 0)
            throw new Error(`unknown collect arguments: ${rest.join(" ")}`);
...
//# sourceMappingURL=agent-control.js.map