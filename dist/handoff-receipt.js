 ...
 */
export function encodeHandoffReceipt(receiptId) {
    const token = `herdr-receipt-v1.${receiptId}`;
    if (!RECEIPT_TOKEN.test(token))
        throw new Error("invalid handoff receipt id");
    return token;
}
export function decodeHandoffReceipt(token) {
    const match = RECEIPT_TOKEN.exec(token);
    if (!match)
        throw new Error("invalid handoff receipt token");
    return match[1].toLowerCase();
}
//# sourceMappingURL=handoff-receipt.js.map