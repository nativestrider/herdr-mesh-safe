...
import { ControllerLeaseStore } from "../lease-store.js";
import { processDescendsFrom, readProcessIdentity } from "../process-attestation.js";
import { ok } from "./types.js";
const SHA256 = /^[0-9a-f]{64}$/i;
const DEFAULT_TTL_SECONDS = 900;
export const defaultControllerAuthorityDependencies = {
...
function controllerProcessIdentity(dependencies) {
    if (dependencies.controllerProcessIdentity)
        return dependencies.controllerProcessIdentity();
    return process.platform === "linux" ? readProcessIdentity(process.ppid) : Promise.resolve(undefined);
}
function requireCallerPaneId(callerPaneId) {
    if (!callerPaneId) {
        throw new Error("HERDR_PANE_ID is required; controller mutations must originate in a Herdr-managed pane");
    }
    return callerPaneId;
}
async function liveAgents(run) {
    const snapshots = [];
    const visit = (node) => {
...
function callerIdentity(agents, callerPaneId) {
...
function identityMatches(lease, snapshot) {
...
export async function currentCallerControllerAuthority(controllerId, dependencies = defaultDependencies) {
    const lease = await dependencies.store.get(controllerId);
    await dependencies.store.assertActive(controllerId, lease.leaseId, lease.fenceToken, dependencies.now());
...
function redact(lease) {
    const { fenceToken: _fenceToken, ...visible } = lease;
    return { ...visible, expired: Date.parse(lease.expiresAt) <= Date.now() };
}
export async function assertControllerAuthority(dependencies, controllerId, leaseId, fenceToken) {
...
export function createSafeControllerTools(dependencies = defaultDependencies) {
    return [
        {
            ...
            run: async (args) => {
                ...
                const ttl = args.ttl_seconds ?? DEFAULT_TTL_SECONDS;
                const lease = {
                    version: 1,
        ...
        },
        {
            ...
            run: async (args) => {
                ...
                if (previous.state !== "active")
                    throw new Error("released controller lease must be acquired, not taken over");
        ...
        },
        {
            ...
            run: async (args) => {
                const controllerId = String(args.controller_id);
                const previous = await assertControllerAuthority(dependencies, controllerId, String(args.controller_lease_id), String(args.controller_fence_token));
                const caller = callerIdentity(await liveAgents(dependencies.run), dependencies.callerPaneId);
                if (!identityMatches(previous, caller))
                    throw new Error("controller identity changed; refusing renewal");
                const now = dependencies.now();
                const lease = {
                    ...previous,
                    renewedAt: now.toISOString(),
                    expiresAt: expiry(now, args.ttl_seconds ?? DEFAULT_TTL_SECONDS),
                };
                await dependencies.store.replace(lease, previous.leaseId);
                return ok(JSON.stringify({ lease }, null, 2));
            },
        },
        {
            ...
            run: async (args) => {
                ...
                const caller = callerIdentity(await liveAgents(dependencies.run), dependencies.callerPaneId);
                if (!identityMatches(previous, caller))
                    throw new Error("controller identity changed; refusing release");
                const now = dependencies.now();
                const lease = {
                    ...previous,
                    state: "released",
