...
export class VerificationRecordStore {
    ...
    async list(controllerId, writerLeaseId) {
        await this.ensureDirectory();
        ...
        for (const name of names) {
            ...
            if (controllerId && record.controllerId !== controllerId)
                continue;
            if (writerLeaseId && record.writerLeaseId !== writerLeaseId)
                continue;
            records.push(record);
        }
        return records;
...
function addDestinationParents(args, destinations) {
    const directories = new Set();
    for (const destination of destinations) {
        let current = dirname(destination);
        while (current !== "/") {
            if (!["/usr", "/etc", "/proc", "/dev", "/tmp"].includes(current))
...
export function buildNetworkResolverBindArgs(resolverPath) {
    if (!isAbsolute(resolverPath))
        throw new Error("network resolver path must be absolute");
    if (resolverPath === "/etc/resolv.conf" || resolverPath.startsWith("/etc/") ||
...
export async function runSandboxedCommand(executable, args, options) {
    ...
    bubblewrapArgs.push("--ro-bind", "/usr", "/usr", "--symlink", "usr/bin", "/bin", "--symlink", "usr/lib", "/lib", "--ro-bind", "/etc", "/etc", "--proc", "/proc", "--dev", "/dev", "--tmpfs", "/tmp");
    if (options.allowNetwork) {
    ...
    addDestinationParents(bubblewrapArgs, (options.readOnlyFiles ?? []).map((file) => file.destination));
    bubblewrapArgs.push("--bind", options.cwd, options.cwd, "--ro-bind", options.gitCommonDir, options.gitCommonDir, "--ro-bind", localBin, localBin, "--ro-bind", uvData, uvData, "--ro-bind", miseData, miseData, "--bind", sandboxHome, "/gate-home", "--clearenv");
    for (const file of options.readOnlyFiles ?? []) {
        bubblewrapArgs.push("--ro-bind", file.source, file.destination);
    }
    for (const [key, value] of Object.entries(options.environment)) {
        if (value !== undefined)
            bubblewrapArgs.push("--setenv", key, value);
    }
...
function commandEnvironment(stateDir, allowNetwork) {
    const environment = {};
...
function safeRequirementsLockPath(value) {
...
function assertHashLockedRequirements(contents) {
    ...
    for (const [index, rawLine] of contents.split("\n").entries()) {
        ...
        if (!logical && (!line || line.startsWith("#")))
            continue;
        ...
        if (!pinned.test(requirement) || hashes.length === 0) {
            throw new Error(`Python bootstrap lock entry ending at line ${index + 1} must be one exact registry pin with SHA-256 hashes`);
        }
        requirementCount += 1;
        logical = "";
    }
...
async function materializeBaseRequirementsLock(git, worktree, baseCommit, lockPath, stateDir, runId) {
...
async function worktreeDigest(git, worktree, baseCommit) {
    ...
    for (const relative of untracked) {
        ...
        if (!absolute.startsWith(`${worktree}${sep}`))
            throw new Error("untracked path escapes the writer worktree");
...
async function inspectWorker(dependencies, controllerId, writerLeaseId, baseCommit, expectedHead, expectedStatusSha256) {
    ...
    if (lease.state !== "active")
        throw new Error(`writer lease is ${lease.state}, not active`);
    if (lease.baseCommit !== baseCommit)
        throw new Error("verification base does not match the writer lease");
    const worktree = await realpath(lease.worktree);
    if (worktree !== lease.worktree)
        throw new Error("writer worktree identity changed");
    const snapshot = extractAgentSnapshot((await dependencies.herdr([
        "agent", "get", lease.agentName,
    ], { timeoutMs: 30_000 })).json);
    if (snapshot.paneId !== lease.paneId || snapshot.name !== lease.agentName ||
    ...
    if (branch !== lease.branch)
        throw new Error(`writer branch is ${branch}, expected ${lease.branch}`);
    const headCommit = await gitText(dependencies.git, ["-C", worktree, "rev-parse", "HEAD"]);
    if (headCommit !== expectedHead)
        throw new Error(`writer HEAD is ${headCommit}, expected ${expectedHead}`);
...
function selectedGate(output) {
    const matches = [...output.matchAll(/^Minimum final gate: just ([A-Za-z0-9_-]+)$/gm)];
    if (matches.length !== 1)
        throw new Error("verification-plan did not report exactly one minimum final gate");
    return matches[0][1];
}
function gateNeedsWeb(gate) {
    return gate !== "check-docs" && gate !== "check-authority";
}
function gateAllowedOnLocalRunner(gate) {
    return gate === "check-docs" || gate === "check-authority" || gate === "check-fast";
}
async function webDependenciesReady(worktree) {
    try {
...
async function hasWebLock(worktree) {
    try {
...
async function finishRecord(dependencies, record, update) {
    const completed = {
        ...record,
        ...update,
...
async function finishFencedRecord(dependencies, record, authority, update) {
    let finalUpdate = update;
    try {
        const active = await assertControllerAuthority(dependencies.controller, authority.controllerId, authority.controllerLeaseId, authority.controllerFenceToken);
...
function boundedCommandTimeout(controllerExpiresAt, now, requestedMs) {
    const available = Date.parse(controllerExpiresAt) - now.getTime() - 5_000;
    if (available <= 0)
        throw new Error("controller lease has too little time remaining for verification");
    return Math.min(requestedMs, available);
}
export function createSafeVerificationTools(dependencies = defaultDependencies) {
    return [
        {
            ...
            run: async (args) => {
                const snapshot = await inspectWorker(dependencies, String(args.controller_id), String(args.lease_id), String(args.base_commit), String(args.expected_head), String(args.expected_status_sha256));
        ...
        },
        {
            ...
            run: async (args) => {
                ...
                const controller = await assertControllerAuthority(dependencies.controller, authority.controllerId, authority.controllerLeaseId, authority.controllerFenceToken);
                const snapshot = await inspectWorker(dependencies, String(args.controller_id), String(args.lease_id), String(args.base_commit), String(args.expected_head), String(args.expected_status_sha256));
                ...
                const pythonLocks = [
                    ...new Set((args.bootstrap_python_locks ?? [])
                ...
                let phase = "preflight";
                try {
                    const plan = await dependencies.command("uv", ["run", "just", "verification-plan", "--base", snapshot.lease.baseCommit], {
                    ...
                    if (gateNeedsWeb(gate) && await hasWebLock(snapshot.lease.worktree) && !await webDependenciesReady(snapshot.lease.worktree)) {
                        ...
                        const bootstrap = await dependencies.command("mise", ["exec", "node@22.22.2", "--", "npm", "--prefix", "web", "ci", "--ignore-scripts"], {
                            cwd: snapshot.lease.worktree,
                            gitCommonDir: snapshot.lease.gitCommonDir,
                            stateDir: dependencies.stateDir,
                            allowNetwork: true,
                    ...
                    for (const lockPath of pythonLocks) {
                        const manifest = await materializeBaseRequirementsLock(dependencies.git, snapshot.lease.worktree, snapshot.lease.baseCommit, lockPath, dependencies.stateDir, runId);
                        const bootstrap = await dependencies.command("uv", [
                    ...
                    const beforeGate = await inspectWorker(dependencies, snapshot.lease.controllerId, snapshot.lease.leaseId, snapshot.lease.baseCommit, snapshot.headCommit, snapshot.gitStatusSha256);
                    ...
                    phase = "gate";
                    const verification = await dependencies.command("uv", ["run", "just", gate], {
                        cwd: snapshot.lease.worktree,
                        gitCommonDir: snapshot.lease.gitCommonDir,
                        stateDir: dependencies.stateDir,
                        allowNetwork: false,
                        timeoutMs: boundedCommandTimeout(controller.expiresAt, dependencies.now(), (args.timeout_seconds ?? 1800) * 1000),
                        environment: commandEnvironment(dependencies.stateDir, false),
                        runId,
                    });
                    phase = "postgate";
                    const after = await inspectWorker(dependencies, snapshot.lease.controllerId, snapshot.lease.leaseId, snapshot.lease.baseCommit, snapshot.headCommit, snapshot.gitStatusSha256);
        ...
        },
        {
            ...
            run: async (args) => ok(JSON.stringify({
                records: await dependencies.records.list(args.controller_id, args.lease_id),
            }, null, 2)),
        },
    ];
}
export const safeVerificationTools = createSafeVerificationTools();
//# sourceMappingURL=safe-verification.js.map