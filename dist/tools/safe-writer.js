...
import { buildAgentStartArgs, buildPaneCloseArgs, buildPaneSplitArgs, extractAgentSnapshot, extractAgentSnapshots, extractPaneId, } from "./safe-agent.js";
...
export function hashGitStatus(status) {
    return createHash("sha256").update(status).digest("hex");
}
export function normalizeOwnershipScopes(scopes) {
    const normalized = scopes.map((scope) => {
        ...
        if (!value || value === ".")
            throw new Error("ownership scopes must identify a repository path");
        return value;
    });
...
export function ownershipScopesOverlap(left, right) {
...
async function inspectGit(git, worktreeInput, baseCommit) {
    if (!isAbsolute(worktreeInput))
        throw new Error("writer worktree must be absolute");
    if (!SHA.test(baseCommit))
        throw new Error("base_commit must be a full Git object id");
    ...
    if (repositoryRoot !== worktree)
        throw new Error("writer worktree must point to the repository root");
    ...
    if (!SHA.test(headCommit))
        throw new Error("writer HEAD is not a full Git object id");
...
function validateExpectedGit(facts, expectedBranch, expectedHead, expectedStatusSha256) {
    if (facts.branch !== expectedBranch)
        throw new Error(`writer branch is ${facts.branch}, expected ${expectedBranch}`);
    if (facts.headCommit !== expectedHead)
        throw new Error(`writer HEAD is ${facts.headCommit}, expected ${expectedHead}`);
...
function validateNoConflict(candidate, leases) {
    for (const lease of leases) {
        if (!retainedWriterStates.has(lease.state) || lease.gitCommonDir !== candidate.gitCommonDir)
            continue;
        if (lease.worktree === candidate.worktree)
            throw new Error(`writer worktree is retained by lease ${lease.leaseId}`);
        if (lease.branch === candidate.branch)
            throw new Error(`writer branch is retained by lease ${lease.leaseId}`);
...
async function validateWorktreeHasNoAgent(herdr, worktree) {
...
async function getWriterAgent(herdr, lease) {
...
async function readWriterOutput(herdr, target, lines) {
    return (await herdr([
        "agent", "read", target, "--source", "visible", "--lines", String(lines), "--format", "text",
    ])).stdout.trim();
}
export function createSafeWriterTools(dependencies = defaultDependencies) {
    return [
        {
            ...
            run: async (args) => {
                ...
                const ownedScopes = normalizeOwnershipScopes(args.owned_scopes);
                const lockedScopes = normalizeOwnershipScopes(args.locked_scopes);
                const protectedBranches = [...new Set(args.protected_branches.map((branch) => branch.trim()))].sort();
                ...
                try {
                    const split = await dependencies.herdr(buildPaneSplitArgs(lease.parentPaneId, String(args.direction ?? "right"), lease.worktree));
                    ...
                    await dependencies.herdr(buildAgentStartArgs(lease.agentName, lease.agentKind, lease.paneId, startTimeout), { timeoutMs: startTimeout + 5000 });
        ...
        },
        {
            ...
            run: async (args) => {
                const { releasing, output } = await dependencies.store.withExclusiveReservation(async () => {
                    ...
                    validateExpectedGit(facts, lease.branch, String(args.expected_head), String(args.expected_status_sha256));
                    const output = await readWriterOutput(dependencies.herdr, lease.agentName, args.read_lines ?? 500);
...
//# sourceMappingURL=safe-writer.js.map