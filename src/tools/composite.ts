...
import { extractAgentSnapshot, waitForSettled, type AgentSnapshot } from "./safe-agent.js";
import { type ToolDef, type ToolResult, ok, targetSchema } from "./types.js";
...
const defaultDependencies: Required<CompositeDependencies> = {
  run: runHerdr,
...
function resolvedDependencies(dependencies: CompositeDependencies): Required<CompositeDependencies> {
  return {
    ...defaultDependencies,
    ...dependencies,
...
function mutationReservations(dependencies: Required<CompositeDependencies>): Reservable[] {
  return [dependencies.controller.store, ...dependencies.leaseStores, dependencies.receiptStore];
}

async function withReservations<T>(
  stores: Reservable[],
  operation: () => Promise<T>,
  index = 0,
): Promise<T> {
...
async function resolveTargets(
  ...
  const resolved = targets.map((target) => {
    ...
    if (matches.length === 0) throw new Error(`agent ${target} has no retained lease`);
    const lease = matches[0];
    if (!lease.leaseId || !lease.paneId) throw new Error(`agent ${target} lease lacks an exact identity`);
    const cwd = lease.cwd ?? lease.worktree;
    if (!lease.agentKind || !cwd) throw new Error(`agent ${target} lease lacks kind or cwd identity`);
...
async function assertNoOutstandingReceipts(store: PromptReceiptStore, targets: string[]): Promise<void> {
  const blocked = (await store.list()).filter(
    (receipt) => targets.includes(receipt.target) && outstandingReceiptStates.has(receipt.state),
  );
  if (blocked.length > 0) {
    throw new Error(
      `agents have outstanding handoffs; collect their receipts first: ${blocked.map((receipt) => receipt.target).join(", ")}`,
    );
  }
}

function receiptToken(receipt: HandoffReceipt): string {
...
async function preflightTargets(
  dependencies: Required<CompositeDependencies>,
  resolved: Map<string, ResolvedTarget>,
): Promise<Map<string, AgentSnapshot>> {
...
async function reserveReceipts(
  ...
  resolved: Map<string, ResolvedTarget>,
  preflight: Map<string, AgentSnapshot>,
): Promise<HandoffReceipt[]> {
  const createdAt = dependencies.now().toISOString();
  const receipts = requests.map((request): HandoffReceipt => {
    const target = resolved.get(request.target);
    const snapshot = preflight.get(request.target);
    if (!target) throw new Error(`target resolution disappeared for ${request.target}`);
    if (!snapshot) throw new Error(`target preflight disappeared for ${request.target}`);
    return {
      version: 1,
  ...
  for (const receipt of receipts) await dependencies.receiptStore.create(receipt);
  return receipts;
}
...
async function failReceipt(
  dependencies: Required<CompositeDependencies>,
  receipt: HandoffReceipt,
  failure: HandoffReceipt["failure"],
): Promise<never> {
...
function assertReceiptSnapshot(
  snapshot: AgentSnapshot,
  receipt: HandoffReceipt,
): "identity_changed" | "cursor_superseded" | undefined {
  if (
    snapshot.name !== receipt.target ||
    snapshot.paneId !== receipt.paneId ||
    snapshot.kind !== receipt.agentKind ||
    !snapshot.cwd || resolve(snapshot.cwd) !== resolve(receipt.cwd)
  ) return "identity_changed";
  if (snapshot.stateChangeSeq !== Number(receipt.afterSeq) + 1) return "cursor_superseded";
  return undefined;
}
...
async function collectOne(
  ...
  if (receipt.state === "completed") {
    ...
    if (
      beforeRead.name !== receipt.target ||
    ...
    const read = await dependencies.run(
      ["agent", "read", receipt.paneId, "--source", "visible", "--lines", String(lines), "--format", "text"],
    ...
    if (
      afterRead.name !== beforeRead.name ||
  ...
  if (invalid) return failReceipt(dependencies, receipt, invalid);
...
async function collectReceipts(
  ...
  if (mode === "all") {
    const outcomes = await Promise.allSettled(
      receipts.map((receipt) => collectOne(dependencies, receipt, timeout, lines)),
    );
    ...
    for (const receipt of receipts) {
      ...
      if (outstandingReceiptStates.has(current.state)) pendingReceipts.push(receiptToken(current));
    }
    return ok(JSON.stringify({ mode, completed, failed, pendingReceipts }, null, 2));
  }

  const controllers = receipts.map(() => new AbortController());
  const tasks = receipts.map((receipt, index) =>
    collectOne(dependencies, receipt, timeout, lines, controllers[index].signal)
  );
  try {
    ...
    const completed = outcomes.flatMap((outcome) => outcome.status === "fulfilled" ? [outcome.value] : []);
    const pendingReceipts: string[] = [];
    const failed: Array<{ target: string; receipt: string; stage: string; error: string }> = [];
...
async function executeBatch(
  ...
  suppliedDependencies: CompositeDependencies,
): Promise<ToolResult> {
  ...
  if (new Set(targets).size !== targets.length) throw new Error("batch targets must be unique");
  await assertControllerAuthority(
    dependencies.controller,
    args.controllerId,
    args.controllerLeaseId,
    args.controllerFenceToken,
  );
  ...
  return withReservations(mutationReservations(dependencies), async () => {
    ...
    const receipts = await reserveReceipts(
      dependencies,
      args.controllerId,
      args.controllerLeaseId,
      args.requests,
    ...
    const delivered = await Promise.all(
      receipts.map((receipt, index) => deliver(dependencies, receipt, args.requests[index].message)),
    );
    ...
    const collected = await collectReceipts(
      dependencies,
      pending,
      args.mode ?? "all",
    ...
    const payload = JSON.parse(collected.content[0].text) as Record<string, unknown>;
    payload.failed = [
      ...((payload.failed as unknown[]) ?? []),
...
export async function collectLeasedTargets(
  args: {
    controllerId: string;
    controllerLeaseId: string;
    controllerFenceToken: string;
  ...
  suppliedDependencies: CompositeDependencies = defaultDependencies,
): Promise<ToolResult> {
  ...
  if (new Set(receiptIds).size !== receiptIds.length) throw new Error("collect receipts must be unique");
  const dependencies = resolvedDependencies(suppliedDependencies);
  await assertControllerAuthority(
    dependencies.controller,
    args.controllerId,
...
async function abandonHandoffReceipt(
  args: {
    controllerId: string;
    controllerLeaseId: string;
    controllerFenceToken: string;
  ...
  return withReservations(mutationReservations(dependencies), async () => {
    ...
    const abandoned: HandoffReceipt = {
      ...receipt,
      state: "failed",
      failure: "abandoned",
      updatedAt: dependencies.now().toISOString(),
...
async function listHandoffReceipts(
...
export function createCompositeTools(suppliedDependencies: CompositeDependencies = defaultDependencies): ToolDef[] {
  return [
    {
      name: "herdr_relay",
      ...
      run: async (a): Promise<ToolResult> => {
        if (a.submit === false) throw new Error("herdr_relay requires submit=true");
        return executeBatch({
    ...
    },
    {
      ...
      timeoutMs: 620_000,
      run: async (a): Promise<ToolResult> => executeBatch({
        controllerId: String(a.controller_id),
        controllerLeaseId: String(a.controller_lease_id),
        controllerFenceToken: String(a.controller_fence_token),
        ...
        readLines: (a.read_lines as number | undefined),
      }, suppliedDependencies),
    },
    {
      ...
      run: async (a): Promise<ToolResult> => executeBatch({
        ...
        controllerLeaseId: String(a.controller_lease_id),
        controllerFenceToken: String(a.controller_fence_token),
        requests: (a.requests as BatchRequest[]).map((request) => ({
          target: String(request.target),
          message: String(request.message),
        })),
        mode: (a.mode as "all" | "first" | undefined),
        timeoutMs: (a.timeout_ms as number | undefined),
        readLines: (a.read_lines as number | undefined),
      }, suppliedDependencies),
    },
    {
      ...
      timeoutMs: 620_000,
      run: async (a): Promise<ToolResult> => collectLeasedTargets({
        controllerId: String(a.controller_id),
        controllerLeaseId: String(a.controller_lease_id),
        controllerFenceToken: String(a.controller_fence_token),
        receipts: (a.receipts as string[]).map(String),
        mode: (a.mode as "all" | "first" | undefined),
        timeoutMs: (a.timeout_ms as number | undefined),
        readLines: (a.read_lines as number | undefined),
      }, suppliedDependencies),
    },
    {
      ...
      run: async (a): Promise<ToolResult> => listHandoffReceipts(
        a.controller_id === undefined ? undefined : String(a.controller_id),
        suppliedDependencies,
      ),
    },
    {
      ...
      run: async (a): Promise<ToolResult> => abandonHandoffReceipt({
        controllerId: String(a.controller_id),
        controllerLeaseId: String(a.controller_lease_id),
        controllerFenceToken: String(a.controller_fence_token),
